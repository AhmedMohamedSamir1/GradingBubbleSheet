    ...
    public static Map<Integer,Object> findBubble(Quadrilateral quad){
        ...
        */
        ...
        for(MatOfPoint c:contours){
            ...
            if( ( bounding_rect.width >= max_n && bounding_rect.height >= max_n &&   ar >= 0.8 && ar <= 1.2 &&
                    bounding_rect.y <= 2780 && bounding_rect.y >= 1280 )){ //awl square USING PAINT MOHEM
        ...
        for (i = 0 ; i < questionContr.size();i+=12){ //8 LW SAFEEN
            //if(i>=132)break;
            List<MatOfPoint> row = questionContr.subList(i, i+12); //8 LW SAFEEN
            sortLeft2Right(row);
            bubbles.addAll(row);
        }
        ...
        for (i = 0 ; i < bubbles.size() ; i += 4 ){
            ...
            if(selection != null){
                ...
                Imgproc.drawContours(quad.trans, Arrays.asList(rows.get(selection[2])), -1, new Scalar(0, 255, 0), 9);
            }/*
            else
            {
        ...
        for(i = 0; i < answer.size(); i++){
            ...
            //answers shayla index of answer of each column in each row
        }
        //answer.clear();
        // answer.addAll(odds);
        // answer.addAll(evens);
        ...
        String[] ans = new String[NumberOfQuestions]; //20
