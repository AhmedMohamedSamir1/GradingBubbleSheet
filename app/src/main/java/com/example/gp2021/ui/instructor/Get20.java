        ...
        */
        ...
        List<Integer> answer = null;
            ...
            for (MatOfPoint c : contours) {
                ...
                if ((bounding_rect.width >= max_n &&
                        ...
                        ar >= 0.7 && ar <= 1.3 &&
                        bounding_rect.y <= 3000 &&
                        bounding_rect.y >= 1393
                        && bounding_rect.x >= 486
                        && bounding_rect.x <= 1970
                )) {

                    questionContr.add(i, c);
                    //Imgproc.drawContours(quad.trans,questionContr,i,new Scalar(164, 148, 242),10);
                    i++;
                }
        ...
        for (i = 0 ; i < bubbles.size() ; i += 4 ){
            ...
            for(int j = 0; j<rows.size();j++){
                ...
                Imgproc.circle(mask, new Point(temp.x+temp.width/2, temp.y+temp.height/2), 50, new Scalar(255, 0, 0), -1);
